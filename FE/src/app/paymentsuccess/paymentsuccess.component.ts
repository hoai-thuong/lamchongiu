...
export class PaymentsuccessComponent implements OnInit {
  ...
  campaignCode: string | null = null; // Add property for campaign code
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.amount = params['amount'] ? `${params['amount']} USD` : 'Không có thông tin';
  ...
  private fetchCampaignCode(donationId: string): void {
    ...
    this.http.get<any>(apiUrl).subscribe({
      ...
      error: (error) => {
        ...
        this.campaignCode = 'Lỗi khi lấy thông tin';
      }
    });
  }
